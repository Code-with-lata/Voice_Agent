        ...
        audio_html = f"""
            ...
            <script>
                (function() {{
                    ...
                    const VOICE_VOLUME_THRESHOLD = 70; 
    ...
    for q in st.session_state.q_bank:
        ...
        if answer_text.strip() in ["...", ".", ""]: # or len(answer_text.split()) < 3:
            ...
            try:
                ...
                scores = re.findall(
                    r'(?:Correctness|Depth|Keywords|Communication):\s*([\d\.]+)', 
                    res_text, 
                    re.IGNORECASE
                )
                if len(scores) >= 4:
                        # Saare scores ko float mein convert karke average nikalna
                    nums = [float(s) for s in scores[:4]]
